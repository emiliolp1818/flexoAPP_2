...
export class ConsultaPedidosComponent implements OnInit {
  ...
  async cargarPedidos() {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (container) {
          container.innerHTML = `<span class="status-icon">✓</span> ${todosPedidos.length} pedidos cargados (actuales + histórico)`;
        }
      }, 0);
    } catch (error) {
      ...
      setTimeout(() => {
        ...
        if (container) {
          container.innerHTML = '<span class="status-icon">✕</span> Error al cargar pedidos';
        }
      }, 0);
    } finally {
  ...
  exportToExcel() {
    ...
    setTimeout(() => {
      ...
      if (container) {
        container.innerHTML = '<span class="status-icon">🔧</span> Función de exportación en desarrollo';
      }
    }, 0);
  }
