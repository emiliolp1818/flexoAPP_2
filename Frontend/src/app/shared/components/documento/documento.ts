...
export class DocumentoComponent implements OnInit {
  ...
  private showMessage(message: string): void {
    ...
    if (message.includes('✓') || message.toLowerCase().includes('correctamente') || message.toLowerCase().includes('exitosamente')) {
      panelClass = 'status-listo-snackbar';
      icon = '✓';
    } else if (message.includes('✗') || message.toLowerCase().includes('error')) {
      panelClass = 'status-terminado-snackbar';
      icon = '✕';
    } else if (message.includes('⬇️') || message.toLowerCase().includes('descargando')) {
      panelClass = 'status-corriendo-snackbar';
      icon = '⬇️';
    } else if (message.includes('🖨️') || message.toLowerCase().includes('imprimiendo') || message.toLowerCase().includes('preparando')) {
      panelClass = 'status-corriendo-snackbar';
      icon = '🖨️';
    } else {
      panelClass = 'status-listo-snackbar';
    ...
    // Limpiar el mensaje de iconos existentes
    ...
    setTimeout(() => {
      ...
      if (container) {
        container.innerHTML = `<span class="status-icon">${icon}</span> ${cleanMessage}`;
      }
    }, 0);
  }
}
