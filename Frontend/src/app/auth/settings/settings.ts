...
export class SettingsComponent implements OnInit, OnDestroy {
  ...
  async loadSystemConfigs() {
    ...
    try {
      ...
      if (response) {
        // Merge: backend configs + frontend defaults que no existan en backend
        const backendIds = new Set(response.map(c => c.id));
        const defaults = this.getDefaultConfigs();
        const missingDefaults = defaults.filter(d => !backendIds.has(d.id));
        const merged = [...response, ...missingDefaults];
        this.systemConfigs.set(merged);
      ...
      this.systemConfigs.set(this.getDefaultConfigs());
    }
  }

  private getDefaultConfigs(): SystemConfig[] {
    return [
        {
          id: 'theme',
          name: 'Tema',
  ...
  getOptionDisplayName(configId: string, option: string): string {
    ...
    const optionMaps: { [key: string]: { [key: string]: string } } = {
      ...
      'alert_red_max': {
        '0': '🔴 0 pedidos', '1': '🔴 1 pedido', '2': '🔴 2 pedidos',
        '3': '🔴 3 pedidos', '4': '🔴 4 pedidos', '5': '🔴 5 pedidos'
      },
      'alert_orange_max': {
        '1': '🟠 1 pedido', '2': '🟠 2 pedidos', '3': '🟠 3 pedidos',
        '4': '🟠 4 pedidos', '5': '🟠 5 pedidos', '6': '🟠 6 pedidos',
        '7': '🟠 7 pedidos', '8': '🟠 8 pedidos'
      },
      'alert_green_min': {
        '2': '🟢 2 pedidos', '3': '🟢 3 pedidos', '4': '🟢 4 pedidos',
        '5': '🟢 5 pedidos', '6': '🟢 6 pedidos', '7': '🟢 7 pedidos',
        '8': '🟢 8 pedidos', '9': '🟢 9 pedidos', '10': '🟢 10 pedidos'
      }
    };
  ...
  getProfileImageUrl(profileImageUrl: string | undefined): string {
    const fullUrl = resolveProfileImageUrl(profileImageUrl);

    if (environment.enableDebugMode && fullUrl) {
  ...
  async togglePermission(permission: Permission, isGranted: boolean) {
    ...
    try {
      ...
      // Si se están editando los permisos del usuario actual, recargar sus permisos
      const currentUserId = currentUser?.id ? parseInt(currentUser.id, 10) : null;
      if (currentUserId === userId) {
        this.permissionsService.loadCurrentUserPermissions(userId).subscribe();
      }
    } catch (error: any) {
