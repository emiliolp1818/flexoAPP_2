...
export class AuthService {
  ...
  private loadStoredUser(): void {
    ...
    if (token && userStr && !this.isTokenExpired(token)) {
      try {
        ...
        if (user && user.id) {
          // Diferir la carga HTTP para evitar NG0200 (inject fuera de contexto)
          queueMicrotask(() => {
            this.permissionsService.loadCurrentUserPermissions(Number(user.id)).subscribe({
  ...
  updateUserProfileImage(userId: string, imageFile: File): Observable<User> {
    ...
    // El backend devuelve { message, profileImage, user } — extraemos el user y el endpoint URL
    return this.http.post<{ message: string; profileImage: string; user: User }>(
      `${environment.apiUrl}/users/${userId}/profile-image`, formData
    ).pipe(
      map(response => {
        // Construir el usuario actualizado con la URL del endpoint de imagen
        const updatedUser: User = {
          ...(response.user || {}),
          profileImage: response.profileImage ?? response.user?.profileImage
        } as User;
