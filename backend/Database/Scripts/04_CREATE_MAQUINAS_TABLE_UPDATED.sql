...
CREATE TABLE IF NOT EXISTS `maquinas` (
    ...
    `orden_excel` INT NOT NULL DEFAULT 0 COMMENT 'Orden de importación del Excel para mantener el orden original',
...
SET @preparedStatement = (SELECT IF(
  ...
  'SELECT ''La columna orden_excel ya existe'' AS resultado;',
  'ALTER TABLE maquinas ADD COLUMN orden_excel INT NOT NULL DEFAULT 0 COMMENT ''Orden de importación del Excel'' AFTER preparando_started_at;'
));
PREPARE alterIfNotExists FROM @preparedStatement;
EXECUTE alterIfNotExists;
DEALLOCATE PREPARE alterIfNotExists;
