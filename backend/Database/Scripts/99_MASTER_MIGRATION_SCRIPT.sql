...
SET @column_exists = (
    ...
    WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'maquinas'
    AND COLUMN_NAME = 'orden_excel'
);

SET @sql = IF(@column_exists = 0,
    'ALTER TABLE maquinas ADD COLUMN orden_excel INT NOT NULL DEFAULT 0 COMMENT ''Orden de importación del Excel''',
    'SELECT ''Columna orden_excel ya existe'' AS resultado'
);
...
-- Relajar constraint de NumeroMaquina (eliminar restricción 11-21)
SET @constraint_exists = (
    SELECT COUNT(*)
    FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS
    WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'maquinas'
    AND CONSTRAINT_NAME = 'chk_maquinas_numero_valido'
);

SET @sql = IF(@constraint_exists > 0,
    'ALTER TABLE maquinas DROP CONSTRAINT chk_maquinas_numero_valido',
    'SELECT ''Constraint chk_maquinas_numero_valido no existe'' AS resultado'
);
...
-- Relajar constraint de Kilos (permitir >= 0)
SET @constraint_exists = (
    SELECT COUNT(*)
    FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS
    WHERE TABLE_SCHEMA = DATABASE()
...
ALTER TABLE maquinas ADD CONSTRAINT chk_maquinas_kilos_positivos CHECK (`Kilos` >= 0);

-- Relajar constraint de NumeroColores (permitir >= 0)
SET @constraint_exists = (
    SELECT COUNT(*)
    FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS
    WHERE TABLE_SCHEMA = DATABASE()
    AND TABLE_NAME = 'maquinas'
    AND CONSTRAINT_NAME = 'chk_maquinas_colores_positivos'
);

SET @sql = IF(@constraint_exists > 0,
    'ALTER TABLE maquinas DROP CONSTRAINT chk_maquinas_colores_positivos',
    'SELECT ''Constraint chk_maquinas_colores_positivos no existe'' AS resultado'
);
...
ALTER TABLE maquinas ADD CONSTRAINT chk_maquinas_colores_positivos CHECK (`NumeroColores` >= 0 AND `NumeroColores` <= 10);
...
SET @sql = IF(@table_exists = 0,
    'CREATE TABLE maquinas_backup (
        ...
        orden_excel INT NOT NULL DEFAULT 0 COMMENT ''Orden de importación del Excel'',
...
-- Agregar columna orden_excel a maquinas_backup si no existe
SET @column_exists = (
    SELECT COUNT(*)
    FROM INFORMATION_SCHEMA.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE()
...
SET @sql = IF(@column_exists = 0,
    'ALTER TABLE maquinas_backup ADD COLUMN orden_excel INT NOT NULL DEFAULT 0 COMMENT ''Orden de importación del Excel''',
    'SELECT ''Columna orden_excel ya existe en maquinas_backup'' AS resultado'
);
...
BEGIN
    ...
    INSERT INTO maquinas_backup (
        ...
        orden_excel, CreatedBy, UpdatedBy, CreatedAt, UpdatedAt,
    ...
    SELECT 
        ...
        orden_excel, created_by, updated_by, created_at, updated_at,
