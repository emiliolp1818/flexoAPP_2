    ...
    public class PermissionsController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<Permission>>> GetAllPermissions()
        {
            try
            {
                ...
                _logger.LogDebug($"📋 {permissions.Count} permisos obtenidos");
        ...
        public async Task<ActionResult<IEnumerable<Permission>>> GetPermissionsByCategory(string category)
        {
            try
            {
                ...
                _logger.LogDebug($"📋 {permissions.Count} permisos obtenidos para categoría '{category}'");
                return Ok(permissions);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult> GetUserPermissions(int userId)
        {
            try
            {
                // Primero verificar usuario (query rápida con proyección)
                var user = await _context.Users
                    .AsNoTracking()
                    .Where(u => u.Id == userId)
                    .Select(u => new { u.UserCode, u.FirstName, u.LastName, u.Role })
                    .FirstOrDefaultAsync();
                ...
                // Queries con AsNoTracking y proyección para evitar serialización de entidades completas
                var allPermissions = await _context.Permissions
                    .AsNoTracking()
                    .Where(p => p.IsActive)
                    .OrderBy(p => p.Category)
                    .ThenBy(p => p.Name)
                    .Select(p => new { p.Id, p.Code, p.Name, p.Category, p.Description, p.IsActive, p.CreatedAt, p.UpdatedAt })
                    .ToListAsync();
                ...
                _logger.LogDebug($"👤 Usuario {userId}: {userPermissions.Count}/{allPermissions.Count} permisos concedidos");
        ...
        public async Task<ActionResult> UpdateUserPermission(int userId, [FromBody] UpdatePermissionRequest request)
        {
            try
            {
                _logger.LogDebug($"🔧 Actualizando permiso '{request.PermissionCode}' para usuario {userId}: {request.IsGranted}");
                ...
                if (userPermission == null)
                {
                    ...
                    _logger.LogDebug($"➕ Creando nuevo permiso para usuario {userId}");
                }
                else
                {
                    ...
                    _logger.LogDebug($"🔄 Actualizando permiso existente para usuario {userId}");
                }

                await _context.SaveChangesAsync();

                _logger.LogDebug($"✅ Permiso '{request.PermissionCode}' {(request.IsGranted ? "concedido" : "revocado")} para usuario {userId}");
        ...
        public async Task<ActionResult> GrantAllPermissions(int userId, [FromBody] GrantAllRequest request)
        {
            try
            {
                _logger.LogDebug($"🔓 Concediendo todos los permisos al usuario {userId}");
                ...
                _logger.LogDebug($"✅ {granted} permisos concedidos al usuario {userId}");
        ...
        public async Task<ActionResult> RevokeAllPermissions(int userId)
        {
            try
            {
                _logger.LogDebug($"🔒 Revocando todos los permisos del usuario {userId}");
                ...
                _logger.LogDebug($"✅ {userPermissions.Count} permisos revocados del usuario {userId}");
