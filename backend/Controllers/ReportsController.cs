...
namespace FlexoAPP.API.Controllers
{
    ...
    public class ReportsController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetRecentActivitiesTest()
        {
            try
            {
                _logger.LogDebug("🧪 TEST: Consultando actividades recientes");
        ...
        public async Task<IActionResult> GetActivitiesStatsTest()
        {
            try
            {
                _logger.LogDebug("🧪 TEST: Consultando estadísticas de Activities");
        ...
        // ============================================================================
        // ENDPOINTS OPTIMIZADOS PARA AUDITORÍA CON PAGINACIÓN
        // ============================================================================

        /// <summary>
        /// Obtiene actividades de auditoría con paginación
        /// Optimizado para manejar grandes volúmenes de datos
        /// </summary>
        [HttpGet("audit/activities/paged")]
        public async Task<ActionResult<PagedResultDto<AuditActivityDto>>> GetAuditActivitiesPaged(
            ...
            [FromQuery] bool sortDescending = true)
        {
            try
            {
                ...
                var filter = new AuditActivityFilterDto
                {
                    UserId = userId,
                    Module = module,
                    Action = action,
                ...
                _logger.LogDebug("📊 Solicitud de actividades paginadas - Página {Page}, Usuario {UserId}, Módulo {Module}", 
                    page, userId, module);

                var result = await _reportsService.GetAuditActivitiesPagedAsync(filter);

                // Log de actividad (sin await para no bloquear)
                _ = Task.Run(async () =>
                {
                    try
                    {
                        await _activityLogger.LogActivityAsync(
                            "VIEW_AUDIT_ACTIVITIES_PAGED",
                            $"Consulta paginada de auditoría - Página {page}",
                            "REPORTS",
                            $"{{\"page\":{page},\"pageSize\":{pageSize},\"totalCount\":{result.TotalCount}}}"
                        );
                    }
                    catch (Exception logEx)
                    {
                        _logger.LogWarning(logEx, "Error registrando actividad");
                    }
                });
        ...
        /// </summary>
        [HttpGet("audit/modules/stats")]
        public async Task<ActionResult<List<ModuleStatsDto>>> GetModuleStats(
            [FromQuery] int? userId = null,
            [FromQuery] DateTime? startDate = null,
            [FromQuery] DateTime? endDate = null)
        {
            try
            {
                ...
                _logger.LogDebug("📊 Solicitud de estadísticas por módulo");

                var stats = await _reportsService.GetModuleStatsAsync(filter);

                return Ok(new
                {
                    success = true,
                    data = stats,
                    message = $"Estadísticas de {stats.Count} módulos obtenidas"
                });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "❌ Error obteniendo estadísticas de módulos");
                return StatusCode(500, new { 
                    success = false, 
                    message = "Error interno del servidor",
                    error = ex.Message 
        ...
        /// <summary>
        /// Obtiene lista de usuarios activos para el filtro
        /// </summary>
        [HttpGet("audit/users/active")]
        public async Task<ActionResult<List<UserBasicDto>>> GetActiveUsers()
        {
            try
            {
                _logger.LogDebug("👥 Solicitud de usuarios activos");

                var users = await _reportsService.GetActiveUsersListAsync();

                return Ok(new
                {
                    success = true,
                    data = users,
                    message = $"{users.Count} usuarios activos encontrados"
                });
            }
            catch (Exception ex)
            {
