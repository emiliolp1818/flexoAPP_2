    ...
    public class MaquinasController : ControllerBase
    {
        ...
        public async Task<ActionResult<object>> GetMaquinas([FromQuery] string? orderBy = "fechaTintaEnMaquina", [FromQuery] string? order = "desc")
        {
            try
            {
                ...
                // Verificar si existe la columna orden_excel
                using var checkCmd = connection.CreateCommand();
                checkCmd.CommandText = @"
                    SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
                    WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'maquinas' AND COLUMN_NAME = 'orden_excel'";
                var hasOrdenExcel = Convert.ToInt32(await checkCmd.ExecuteScalarAsync()) > 0;

                var selectOrden = hasOrdenExcel ? ", orden_excel" : "";
                ...
                command.CommandText = $@"
                    SELECT
                        ...
                        created_by, updated_by, created_at, updated_at{selectOrden}
                    FROM maquinas
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (!exists)
                    {
                        ...
                        _logger.LogInformation("✅ Columna orden_excel creada en la tabla maquinas");
                    }
                }
                ...
                _logger.LogInformation("🧹 Iniciando limpieza de programas TERMINADOS y SIN ASIGNAR...");
                {
                    ...
                    _logger.LogInformation($"🗑️ {countTerminados} programas TERMINADOS eliminados");
                    ...
                    _logger.LogInformation($"🗑️ {countSinAsignar} programas SIN ASIGNAR eliminados");

                    _logger.LogInformation($"✅ Limpieza completada. Total eliminados: {countTerminados + countSinAsignar}");
                    ...
                    try
                    {
                        ...
                        if (Convert.ToInt32(await resetCmd.ExecuteScalarAsync()) > 0)
                        {
                            ...
                            _logger.LogInformation("🔄 Orden de programas protegidos reseteado a 0");
                        }
                    }
        ...
        private async Task<ImportSheetResult> ProcessWorksheet(ExcelWorksheet worksheet, int machineNumber, MySqlConnector.MySqlConnection designConn)
        {
            ...
            try
            {
                _logger.LogInformation($"💾 Guardando {maquinasToAdd.Count} nuevos registros y {maquinasToUpdate.Count} actualizaciones en orden...");
                ...
                await _context.SaveChangesAsync();
                _logger.LogInformation($"✅ Guardado exitoso: {maquinasToAdd.Count} creados, {maquinasToUpdate.Count} actualizados");
            }
            catch (Exception ex)
            {
        ...
        private async Task<ImportSheetResult> ProcessProgramaCCWorksheet(ExcelWorksheet worksheet, MySqlConnector.MySqlConnection designConn)
        {
            ...
            _logger.LogInformation($"🛡️ {protectedPrograms.Count} programas con estados protegidos encontrados");
            ...
            try
            {
                ...
                for (int row = 6; row <= rowCount; row++)
                {
                    try
                    {
                        ...
                        if (string.IsNullOrEmpty(otSap))
                        {
                            ...
                            otSap = $"INFO-R{row}-M{mq ?? "0"}-{DateTime.UtcNow:HHmmss}";
                        ...
                        {
                            _logger.LogInformation($"ℹ️ Fila {row}: Fila informativa detectada - MQ={mq}, Art={articulo}, OT={otSap}, Cli={cliente}");
                        }
                        ...
                        if (isUpdate)
                        {
                            ...
                            updateCmd.Parameters.AddWithValue("@updAt", DateTime.UtcNow);
                        ...
                        else
                        {
                            ...
                            insertCmd.Parameters.AddWithValue("@creAt", DateTime.UtcNow);
                            insertCmd.Parameters.AddWithValue("@updAt", DateTime.UtcNow);
                ...
                _logger.LogInformation($"✅ Transacción completada: {result.Created} creados, {result.Updated} actualizados");
            }
            catch (Exception ex)
            {
            ...
            _logger.LogInformation($"📊 Completado: {result.Created} creados, {result.Updated} actualizados, {result.Errors} errores, {skippedRows} ignoradas");
            return result;
        }
