...
namespace FlexoAPP.API.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        private readonly Services.Implementations.LoginRateLimiterService _rateLimiter;


        public AuthController(
            IAuthService authService,
            IActivityLoggerService activityLogger,
            Services.Implementations.LoginRateLimiterService rateLimiter)
        {
        ...
        public async Task<IActionResult> Login([FromBody] LoginDto loginDto)
        {
            try
            {
                ...
                var rateLimitKey = Services.Implementations.LoginRateLimiterService.GetKey(loginDto?.UserCode, ipAddress);

                // Verificar si está bloqueado por intentos fallidos
                var (isLocked, secondsRemaining) = _rateLimiter.IsLockedOut(rateLimitKey);
                if (isLocked)
                {
                ...
                var (locked, attemptsRemaining, lockSeconds) = _rateLimiter.RegisterFailedAttempt(rateLimitKey);
                if (locked)
                {
                    return StatusCode(429, new
                    {
                        message = "Demasiados intentos fallidos. Cuenta bloqueada por 5 minutos.",
                        lockedUntilSeconds = lockSeconds,
                        locked = true
                    });
                }
                ...
                {
                    message = "Código de usuario o contraseña incorrectos",
