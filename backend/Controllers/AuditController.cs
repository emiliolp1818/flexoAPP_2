    ...
    public class AuditController : ControllerBase
    {
            ...
            [FromQuery] int pageSize = 50)
        {
            try
            {
                ...
                _logger.LogDebug($"🔍 GetActivities llamado con parámetros: userId={userId}, module={module}, action={action}, startDate={startDate}, endDate={endDate}");
                ...
                if (userId.HasValue)
                {
                    _logger.LogDebug($"✅ Aplicando filtro de userId: {userId.Value}");
                    query = query.Where(a => a.UserId == userId.Value);
                }

                if (!string.IsNullOrEmpty(module))
                {
                    _logger.LogDebug($"✅ Aplicando filtro de module: {module}");
                    query = query.Where(a => a.Module == module);
                }

                if (!string.IsNullOrEmpty(action))
                {
                    _logger.LogDebug($"✅ Aplicando filtro de action: {action}");
                    query = query.Where(a => a.Action == action);
                }

                if (startDate.HasValue)
                {
                    _logger.LogDebug($"✅ Aplicando filtro de startDate: {startDate.Value}");
                    query = query.Where(a => a.Timestamp >= startDate.Value);
                }

                if (endDate.HasValue)
                {
                    _logger.LogDebug($"✅ Aplicando filtro de endDate: {endDate.Value}");
                ...
                _logger.LogDebug($"📊 Total de registros después de filtros: {totalRecords}");
                ...
                _logger.LogDebug($"📤 Enviando {activities.Count} actividades al frontend");
                ...
                if (userId.HasValue && activities.Any())
                {
                    ...
                    {
                        _logger.LogDebug($"✅ Todas las actividades son del usuario {userId.Value}");
                    }
                }
        ...
        public async Task<ActionResult> DeleteActivity(int id)
        {
            try
            {
                ...
                return Ok(new { success = true, message = "Actividad eliminada" });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error eliminando actividad {Id}", id);
                return StatusCode(500, new { message = "Error al eliminar actividad" });
            }
        }
    }
}
